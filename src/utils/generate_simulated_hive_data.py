...
def hive_1_weight_func(h):
    ...
    @returns {np.ndarray} - The weights of the simulated hive 1 at the specified hours. This
        function returns the sum of two sinusoids (one sinusoid where the period equals one day and
        one sinusoid where the period equals one year) plus a positive offset, so that the hive
        weight never goes below zero.
    """
...
def hive_2_weight_func(h):
    ...
    @returns {np.ndarray} - The weights of the simulated hive 2 at the specified hours. This
        function returns the sum of two sinusoids (one sinusoid where the period equals one day and
        one sinusoid where the period equals one year) plus a positive offset, so that the hive
        weight never goes below zero.
    """
...
def generate_simulated_sensor_data(insert_to_mongodb):
    ...
    hive_1_weight_over_time = hive_1_weight_func(
        hours)  # hive 1 weight for each hour in a year
    hive_2_weight_over_time = hive_2_weight_func(
        hours)  # hive 2 weight for each hour in a year
